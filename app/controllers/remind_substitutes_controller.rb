class RemindSubstitutesController < ApplicationController
	...
	def index
		...
		@remind_substitutes = Substitute.request_type_planned.status_is_needtoconfirm.search(params[:search_by], params[:search]).page(params[:page]).per(10)
	...
	def show
		...
		@remind_substitute_details = @remind_substitute.details.order('date_applied ASC').where(deleted: false)
	end
